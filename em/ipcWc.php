 ...
 */
		...
		for ($k = 0; $k < $wcObj->nodes; $k++) {
			...
			for ($n = 0; $n < count($nodeinfo); $n++) {
				if($nodeinfo[$n]['type'] == "MIOX" ) {
					$miox[] = ['node' => $node, 'psta' => $nodeinfo[$n]['psta'], 'type' => $nodeinfo[$n]['type']];
				}
				if ($nodeinfo[$n]['type'] == "MIOY") {
					$mioy[] = ['node' => $node, 'psta' => $nodeinfo[$n]['psta'], 'type' => $nodeinfo[$n]['type']];
				}
	...
	function holdWc($userObj, $wcObj, $wcname, $wcc, $clli, $npanxx, $frloc) {
		...
		$wcObj->holdWc($wcname, $wcc, $clli, $npanxx, $frloc);
		if($wcObj->rslt == FAIL) {
			$result["rslt"] = $wcObj->rslt;
        	$result["reason"] = $wcObj->reason;
			$result["rows"] = $wcObj->rows;

...
class CMD {
    ...
    public function getCmdList($cmdBlock) {
        global $db;
        
        $this->rows = [];
        $qry = "SELECT * FROM t_cmdque WHERE stat <> 'COMPL' limit $cmdBlock";
        $res = $db->query($qry);
        if (!$res) {
            $this->rslt = "fail";
    ...
    public function createCmd($act, $pathId, $path) {
        ...
        for($i=0; $i < count($relayArray); $i++) {
            ...
            $rcObj->queryRC($relay);    //Do we need to check error of this query

...
class MXC {
    ...
    public function queryByType($type) {
        global $db;
        $qry = "SELECT * FROM t_mxc WHERE type = '$type' AND node <= '$this->nodes'";
    ...
    public function getStatByType($type) {
        global $db;
        $qry = "SELECT node, slot, type, psta FROM t_mxc WHERE type = '$type' AND node <= '$this->nodes'";
